import random
from player_notes import Player_notes
...
class Game: 
     ...
     #give a clue? 
          result = "correct"
          if incorrect != []:
               result = random.choice(incorrect)
               print(f"You realize {result} couldn't be it..by jove, it's all coming together!")
          else:
               print("You're on the right track!")
          self.evidence.add_guess(Guess(suspect, weapon, room, result))

     def display_evidence(self):
          self.evidence.display_guesses()
